...
public interface IClassroomService
{
    ...
    /// <summary>
    /// Tüm sınıf (classroom) kayıtlarını admin areada listeleme sayfası (index) için getirir.(Sadece ID ve Name)
    /// </summary>
    Task<IDataResult<List<ClassroomFilterDto>>> GetAllForIndexAsync();
    ...
    /// <returns></returns>
    Task<IDataResult<List<ClassroomListDto>>> GetFilteredByNameOrBranchNameOrGroupTypeOrOpeningDateOrClosedDateAsync(string name, string branchName, string groupType, DateTime openingDate, DateTime closedDate,int takeCount);
    ...
    /// </summary>
    Task<IResult> MakePassiveAsync(Guid id);


    Task<IDataResult<List<ClassroomListDto>>> GetActivelassroomsByStudentIdAsync(Guid studentId);

    Task<IDataResult<List<ClassroomListDto>>> GetClassroomsForAutoComplete(string term);
}
