    ...
    public TrainerClassroomProfile()
    {
      ...
      .ReverseMap();
        ...
        CreateMap<TrainerClassroom, TrainerClassroomListForTrainerDetailsDto>()
            ...
            .ForMember(dest => dest.StudentCount, opt => opt.MapFrom(src => src.Classroom.StudentClassrooms.Where(sc=>sc.Status != Core.Enums.Status.Deleted).DistinctBy(sc=>sc.StudentId).Count()));
