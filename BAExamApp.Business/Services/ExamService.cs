...
public class ExamService : IExamService
{
    ...
    public async Task<IDataResult<List<ExamListDto>>> GetExamsByStatusAsync(string status)
    {
        ...
        if (status == "past")
        {
            filteredExams = exams.Where(e => (e.ExamDateTime + e.ExamDuration) < currentDate).ToList();
        }
        else if (status == "ongoing")
        {
            filteredExams = exams.Where(e => e.ExamDateTime <= currentDate && currentDate <= (e.ExamDateTime + e.ExamDuration)).ToList();
        }
        else if (status == "upcoming")
        {
            filteredExams = exams.Where(e => e.ExamDateTime > currentDate).ToList();
        }
        else
        {
            filteredExams = exams.ToList();
        }

        var examListDtos = _mapper.Map<List<ExamListDto>>(filteredExams);

        foreach (var examDto in examListDtos)
        {
            var examEntity = filteredExams.FirstOrDefault(e => e.Id == examDto.Id);

            if (examEntity != null && examEntity.StudentExams != null)
            {
                examDto.StudentExamScoreCount = examEntity.StudentExams.Count(x => x.Score != null);
                examDto.StudentCount = examEntity.StudentExams.Count();
            }
            else
            {
                examDto.StudentExamScoreCount = 0;
                examDto.StudentCount = 0;
            }
        }

        return new SuccessDataResult<List<ExamListDto>>(examListDtos, Messages.ListedSuccess);
    }
    ...
    public async Task<IDataResult<List<ExamListDto>>> GetExamsByFiltered(string selectedClassroomId, string selectedRulenameId, string datetimePickerStart, string datetimePickerEnd, bool isActiveExams)
    {
        ...
        else
        {
            ...
            (string.IsNullOrEmpty(datetimePickerEnd) || (x.ExamDateTime + x.ExamDuration) < ParseDateTimeWithT(datetimePickerEnd, setToEndOfDay: true)));

            return new SuccessDataResult<List<ExamListDto>>(_mapper.Map<List<ExamListDto>>(filteredExams), Messages.ListedSuccess);
        }
        DateTime ParseDateTimeWithT(string dateTimeString, bool setToEndOfDay = false)
        {
            if (DateTime.TryParseExact(dateTimeString, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime parsedDateTime))
            {
                if (setToEndOfDay)
                {
                    return parsedDateTime.AddDays(1).AddSeconds(-1);
                }
