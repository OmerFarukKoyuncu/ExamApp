...
public class StudentExamService : IStudentExamService
{
    ...
    /// <summary>
    /// Öğrenci kimliğine göre sınav kayıtlarını getirir. Kayıt yoksa hata mesajı döner.
    /// </summary>
    /// <param name="id">Öğrencinin benzersiz ID'si.</param>
    /// <returns>Sınav listesiyle birlikte işlem sonucu.</returns>
    public async Task<IDataResult<List<StudentsExamsDto>>> GetExamListByStudentIdAsync(Guid id)
    {
        var studentExams = await _studentExamRepository.GetAllAsync(x => x.StudentId == id);

        if (studentExams.Count() == 0)
        {
            return new ErrorDataResult<List<StudentsExamsDto>>(Messages.StudentExamsNotFound);
        }

        return new SuccessDataResult<List<StudentsExamsDto>>(_mapper.Map<List<StudentsExamsDto>>(studentExams), Messages.FoundSuccess);
    }
