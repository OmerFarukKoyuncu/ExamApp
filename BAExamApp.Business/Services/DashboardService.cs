...
public class DashboardService : IDashboardService
{
    ...
    /// <summary>
    /// Belirli bir eğitmenin oluşturduğu veya güncellediği, silinmemiş ve belirli durumlara sahip olan
    /// en yeni sorulardan belirtilen sayıda (takeCount) adedini getirir.
    /// </summary>
    /// <param name="trainerIdentityId">Sorgulanan eğitmenin Identity ID değeri.</param>
    /// <param name="takeCount">Getirilecek maksimum soru sayısı.</param>
    /// <returns>
    /// Eğitmene ait soruların DTO listesi ve işlem sonucunu içeren veri nesnesi.
    /// </returns>
    public async Task<IDataResult<List<DashboardTrainerQuestionDto>>> GetWaitedRevisedApprovedQuestionByTrainerIdAsync(
        string trainerIdentityId, int takeCount)
    {
        var questions = await _questionRepository.GetAllAsync(
            question => (question.CreatedBy == trainerIdentityId || question.ModifiedBy == trainerIdentityId) &&
                        (question.State == State.Awaited || question.State == State.Approved ||
                         question.State == State.Reviewed) &&
                        question.Status != Status.Deleted,
            question => question.CreatedDate,
            0,
            takeCount,
            true,
            false
        ...
        var mapped = _mapper.Map<List<DashboardTrainerQuestionDto>>(questions);

        return new SuccessDataResult<List<DashboardTrainerQuestionDto>>(mapped, Messages.ListedSuccess);
    }
    ...
    /// <summary>
    /// Veritabanındaki bekleyen (awaited) sorulardan, silinmemiş olanlardan, belirtilen sayıda (takeCount) en yeni ilk 10 soruyu getirir.
    /// </summary>
    /// <param name="takeCount">Alınacak soru sayısı.</param>
    /// <returns>Başarılı bir şekilde alınan soruları içeren bir IDataResult döndürür.</returns>

    public async Task<IDataResult<List<QuestionListDto>>> GetTopTenByAwaitedQuestionAsync(
      int takeCount)
    {

        var questions = await _questionRepository.GetAllAsync(
            question =>
                        (question.State == State.Awaited) &&(question.Status!=Status.Passive)
                        &&
                        question.Status != Status.Deleted,
            question => question.CreatedDate,
            0,
    ...
    /// <summary>
    /// Veritabanındaki bekleyen (reviewed) sorulardan, silinmemiş olanlardan, belirtilen sayıda (takeCount) en yeni ilk 10 soruyu getirir.
    /// </summary>
    /// <param name="takeCount">Alınacak soru sayısı.</param>
    /// <returns>Başarılı bir şekilde alınan soruları içeren bir IDataResult döndürür.</returns>
    public async Task<IDataResult<List<QuestionListDto>>> GetTopTenByReviewedQuestionAsync(
     int takeCount)
    {

        var questions = await _questionRepository.GetAllAsync(
            question =>
                        (question.State == State.Reviewed) && (question.Status != Status.Passive)
                        &&
                        question.Status != Status.Deleted,
            question => question.CreatedDate,
            0,
