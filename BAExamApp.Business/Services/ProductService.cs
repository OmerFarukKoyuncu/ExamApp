...
public class ProductService : IProductService
{
    ...
    public async Task<IResult> DeleteAsync(Guid id)
    {
        ...
        var productSubjectUsingProduct = await IsProductSubjectUsedProductAsync(product.Id);

        if (classroomUsingProduct)
        {
            var classroomProdcuts = await _classroomProductRepository.GetAllAsync(x => x.ProductId == product.Id);
            foreach (var classroomProdcut in classroomProdcuts)
            {
                await _classroomProductRepository.DeleteAsync(classroomProdcut);
            }
            await _classroomProductRepository.SaveChangesAsync();
        }

        if (trainerUsingProduct)
        {
            var trainerProdcuts = await _trainerProductRepository.GetAllAsync(x => x.ProductId == product.Id);
            foreach (var trainerProdcut in trainerProdcuts)
            {
                await _trainerProductRepository.DeleteAsync(trainerProdcut);
            }
            await _trainerProductRepository.SaveChangesAsync();
        }

        if (examRuleUsingProduct)
        {
            var examRuleProdcuts = await _examRuleRepository.GetAllAsync(x => x.ProductId == product.Id);
            foreach (var examRuleProdcut in examRuleProdcuts)
            {
                await _examRuleRepository.DeleteAsync(examRuleProdcut);
            }
            await _examRuleRepository.SaveChangesAsync();
        }

        if (productSubjectUsingProduct)
        {
            var productSubjectProducts = await _productSubjectRepository.GetAllAsync(x => x.ProductId == product.Id);
            foreach (var productSubjectProduct in productSubjectProducts)
            {
                await _productSubjectRepository.DeleteAsync(productSubjectProduct);
            }
            await _productSubjectRepository.SaveChangesAsync();
        }
    ...
    public async Task<bool> IsProductSubjectUsedProductAsync(Guid productId)
    {
        var productSubject= await _productSubjectRepository.AnyAsync(e => e.ProductId == productId);
