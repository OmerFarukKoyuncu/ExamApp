...
[BreadcrumbName("Question_Procedures", false)]
public class QuestionController : TrainerBaseController
{
    ...
    public async Task<IActionResult> QuestionList(State state, int? page, int pageSize = 10, bool showAllQuestions = false)
    {
        ...
        _breadcrumbService.AddBreadcrumb(new BreadcrumbItemDto
        {
            ...
            Url = $"/Question_Procedures/QuestionList?state={state}",
            IsActive = true
        });
    ...
    public async Task<IActionResult> CopyQuestion(Guid id)
    {
        ...
        questionCreateVM.SubtopicId = questionResult.Data.SubtopicId.Select(x=>x.SubtopicId).ToList();
