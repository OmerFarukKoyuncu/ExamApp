...
[BreadcrumbName("Exam_Operations",false)]
public class ExamController : TrainerBaseController
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        return View(new X.PagedList.PagedList<TrainerExamListVM>(new List<TrainerExamListVM>(), 1, 10));
    }
    ...
    /// <returns></returns>
    [HttpPost]
    public async Task<IActionResult> GetExamsByStatus(string className, string ruleName, string startDate, string endDate, bool isActiveExams, int page = 1, int pageSize = 10)
    {
        ...
        return RedirectToAction("GetExamsByStatus", new { page = page, pageSize = pageSize });
    }

    [HttpGet]
    public async Task<IActionResult> GetExamsByStatus(int page = 1, int pageSize = 10)
    {
        if (_memoryCache.TryGetValue("trainerExamList", out var cachedList))
        {
            ...
            var examList = ((IEnumerable<TrainerExamListVM>)cachedList).ToList();
            ...
            var pagedList = examList.ToPagedList(page, pageSize);
    ...
    {
        var examDeleteResponse = await _examService.SoftDeleteAsync(id);
    ...
    [BreadcrumbName("Create_Exam")]
    [HttpGet]
    public async Task<IActionResult> Create()
    {
        var examRules = await GetExamRules();

        if (examRules.Count > 0)
        {
    ...
    [BreadcrumbName("Exam_Evaluator_List")]
    [HttpGet]
