...
public class ExamRuleController : AdminBaseController
{
    ...
    public async Task<IActionResult> Index(string typeOfExamRule, int? page, int pageSize = 10, bool? showAllData = null)
    {
        ...
        ViewBag.QuestionTypeList = GetQuestionTypes("1");
    ...
    public async Task<IActionResult> Update(Guid id)
    {
        ...
        ViewBag.ProductList = await GetProductsAsync();
        ViewBag.SubjectList = await GetSubjectsByProductId(examRuleUpdateVM.ProductId.ToString());
        ViewBag.QuestionTypeList = GetQuestionTypes("1");
    ...
    [HttpGet]
    public async Task<IActionResult> SearchExamRuleSearchAutocomplete(string term)
    {
        if (string.IsNullOrWhiteSpace(term))
            return Json(new List<string>());

        var examRules = await _examRuleManager.GetAllAsync();
        var matched = examRules.Data
