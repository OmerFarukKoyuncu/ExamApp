...
public class QuestionController : AdminBaseController
{
    ...
    public async Task<IActionResult> QuestionList(State state, int? page, int pageSize = 10, bool showAllQuestions = false)
    {
        ...
        //Breadcrumb'ı manuel moda geçir.
        _breadcrumbService.SwitchToManual();
        _breadcrumbService.AddBreadcrumb(new BreadcrumbItemDto
        {
            Title = "Question",
        });
        
        //İLgili duruma göre breadcrumb hazırla.
        var entityName = _stringLocalizer["Question"]; // "Soru" veya "Question"
        var localizedTitle = _stringLocalizer[$"{state}_List", entityName];
        _breadcrumbService.AddBreadcrumb(new BreadcrumbItemDto
        {
            Title = localizedTitle,
            Url = $"/Question/QuestionList?state={state}",
            IsActive = true
        });
    ...
    public async Task<IActionResult> QuestionDetails(Guid id, int? page, int pageSize = 10, bool showAllQuestions = false)
    {
        ...
        var questionRevisions = await _questionRevisionService.GetAllRevisionsByQuestionId(id);
                ...
                if (questionResult.IsSuccess)
                {
                    ...
                    if (questionAnswersResult.IsSuccess)
                    {
                        ...
                        else
                        {
                            ...
                            await _questionRevisionService.AddArrangeAsync(questionArrange);
    ...
    {
        var questionArranges = await _questionRevisionService.GetAllArrangesByQuestionIdAsync(Guid.Parse(questionId));
    ...
    [HttpGet]
    [BreadcrumbName("Create_Question")]
