...
public class ProductController : AdminBaseController
{
    ...
    {
        //if (showAllData == null && HttpContext.Session.GetInt32("ShowAllData") != null) // diğer sayfalarda da seçili gelmemesi için kaldırıldı
        //{
        //    showAllData = HttpContext.Session.GetInt32("ShowAllData") == 1;
        //}

        bool showAll = showAllData ?? false;
        //HttpContext.Session.SetInt32("ShowAllData", showAll ? 1 : 0); // diğer sayfalarda da seçili gelmemesi için kaldırıldı
    ...
    public async Task<IActionResult> Details(Guid id)
    {
        ...
        if (product.IsSuccess)
        {
            ...
            foreach (var trainerProduct in mappedProduct.TrainerProducts)
            {
                ...
                if (trainer.IsSuccess && trainer.Data != null)
                {
                    trainerProduct.FullName = trainer.Data.FirstName + " " + trainer.Data.LastName;
                }
                else
                {
                    trainerProduct.FullName = "Eğitmen bulunmamaktadır."; 
                }
            }
    ...
    [HttpGet]
    public async Task<IActionResult> SearchProductAutocomplete(string term)
    {
