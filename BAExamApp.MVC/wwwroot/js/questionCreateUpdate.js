...
//Product type change event.
async function onSubjectChange(selectedSubtopicGuids = []) {
    var subjectId = $("#SubjectId").val();
    selectedSubtopicGuids = selectedSubtopicGuids || [];

    if (!subjectId) return;

    getSubtopics(subjectId).then(subtopics => {
        populateSelectList("SubtopicId", subtopics);
        removeChooseOption("SubtopicId");

        if (selectedSubtopicGuids.length > 0) {
            let matchedSubtopics = subtopics
                .filter(s => selectedSubtopicGuids.includes(s.guid))
                .map(s => s.id);

            if (matchedSubtopics.length > 0) {
                $("#SubtopicId").val(matchedSubtopics).trigger("change.select2");
            } else {
                console.warn("Seçili alt konu bulunamadı:", selectedSubtopicGuids);
            }
        }
    });
}
...
async function getAnswerChoicesHtml(choiceType) {
    ...
    if (answerTypeName === "Metin" || answerTypeName === localizer.text) {
        ...
        answerChoicesHtml += questionAnswerChoices.map((item, index) => {
            return `
        <div class="form-check mb-3" id="choices">
            <div class="row g-3 flex-center justify-content-center">
                <div class="form-check col-sm-1 col-1 offset-xs-5 offset-sm-0">
                    <input class="form-check-input" type="${choiceType}" name="answerOptions" id="choice${index}" ${item.IsRightAnswer ? 'checked' : ''} onChange="updateCheckedAnswers()">
                </div>
                <div class="col-sm-9 col-12">
                    <div style="background-color: #f8f9fa; border-radius: 0.475rem; padding: 10px; display: flex; align-items: center; width: 100%;">
                        <button type="button" class="btn btn-secondary" onclick="triggerFileInput(${index})">
                            ${(localizer.chooseFile)}
                        </button>

                        <span id="file-name-${index}" style="margin-left: 10px; flex-grow: 1; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">
                            ${(localizer.nofileselected)}
                        </span>

                        <img id="image-preview-${index}" style="margin-left: 10px; width: 40px; height: 40px; object-fit: contain; display: none;" />

                        <button class="btn btn-danger btn-sm" style="margin-left: 10px;" type="button" onclick="removeChoice(${index},'${choiceType}')">X</button>

                        <input type="file" id="file-input-${index}" style="display: none;" accept=".png,.jpeg" onchange="setupFileInput(${index})" />
                    </div>
                </div>
            </div>
        </div>`;
...
function setupFileInput(index) {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            ...
            questionAnswerChoices[index].Answer = e.target.result;
            questionAnswerChoices[index].IsAnswerImage = true;
            document.getElementById("questionAnswerChoices").value = JSON.stringify(questionAnswerChoices);
        };
        reader.readAsDataURL(file);
    } else {
        fileNameSpan.textContent = (localizer.nofileselected) ? localizer.nofileselected : 'No file selected';
        imagePreview.src = "";
